...
## there are 12 max mixscore ADM vectors, each contains 1001 max values of mixscores, the first one is the unshuffled dataset's max value
max.vectors <- rep(NA, 12)
...
for( i in 1:12){
  ...
  max.vectors[i] <- scan(file.name, what = character() )
